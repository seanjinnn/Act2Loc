...
class MarkovDiaryGenerator:
    def __init__(self):
        ...
        self._time_slot_length_ = '1H'

    # 1. initialize markov chain
    def _create_empty_markov_chain_(self):
    ...
    # 2. create the time series of the individual
    def _get_location2frequency(self, traj, location_column='location'):
        location2frequency, location2rank = defaultdict(int), defaultdict(int)

        # location2freq
        for i, row in traj.iterrows():
            if isinstance(row[location_column], str):  # traj['cluster'] != np.nan
                for location in row[location_column].split(','):  # split(','): location may belong to different cluster
                    location2frequency[location] += 1

        # location2rank
        rank = 1
        # location2frequency.items(): [(key,value), ..., (key,value)]
        # operator.itemgetter(1): sort list by second value of each pair
    ...
    # cluster select
    def _select_loc(self, traj, location2frequency, location_column='location'):
        if isinstance(traj[location_column], str):  # Not Nan
            locations = traj[location_column].split(',')

            # cluster: 1; multiple locations, same frequency; multiple location, different frequencies
            if len(locations) == 1:
                return locations[0]
            elif len(set(Counter(locations).values())) != 1:
                return sorted(Counter(locations).items(), key=operator.itemgetter(1), reverse=True)[0][0]
            else:
                fre_dict = {k: location2frequency[k] for k in locations}
                return sorted(fre_dict.items(), key=operator.itemgetter(1), reverse=True)[0][
                    0]  # locations with the highest overall frequency
        return np.nan

    def _create_time_series(self, traj, lid='location'):  # lid = cluster
        shift = traj['datetime'].min().hour
        traj = traj[['datetime', lid]].set_index('datetime')  # 获取'datetime' 'cluster'两列并按照datetime编号
        traj[lid] = traj[lid].astype('str')  # 'cluster'转str格式

        # closed提示freq列位置(left='datetime' or right='cluster')
        traj = traj.groupby(pd.Grouper(freq=self._time_slot_length_, closed='left')).agg(lambda x: ','.join(x)).replace(
            '', np.nan)

        # cluster.unique()
        l2f, l2r = self._get_location2frequency(traj, location_column=lid)
        time_series = traj.apply(lambda x: self._select_loc(x, location2frequency=l2f, location_column=lid),
                                 axis=1)  # axis=1: column
        ...
        time_series = time_series.apply(lambda x: l2r[x])

        return time_series, shift

    # 3. update the markov chain via time_series
    def _update_markov_chain_(self, time_series, shift=0):
        ...
        while slot < n - 1:
            ...
            loc_h = time_series[slot]  # current and next slot of abstract location
            next_loc_h = time_series[slot + 1]

            if loc_h == 1:
                if next_loc_h == 1:  # [1,1]
                    self._markov_chain_[(h, 1)][(next_h, 1)] += 1  # (h,1) -> (h+1,1)
                else:  # [1,0]
                    tau = 1  # (h,1) -> (h+tau,0)
                    if slot + 2 < n:
                        for j in range(slot + 2, n):
                            loc_hh = time_series[j]
                            if loc_hh == next_loc_h:
                                tau += 1
                            else:
                                break

                        h_tau = (h + tau) % 24
                        self._markov_chain_[(h, 1)][(h + tau, 0)] += 1
                        slot = j - 2
                    else:
                        slot = n
            else:
                if next_loc_h == 0:  # [0,1]
                    self._markov_chain_[(h, 0)][(next_loc_h, 1)] += 1  # (h,0) -> (h+1,1)
                else:  # [0,0]
                    tau = 1  # (h,0) -> (h+tau,0)
                    if slot + 2 < n:
                        for j in range(slot + 2, n):
                        ...
                        h_tau = (h + tau) % 24
                        self._markov_chain_[(h, 0)][(next_h, 1)] += 1
                        slot = j - 2
                    else:
                        slot = n
            slot += 1

    # 4. normalize the markov chain by row
    def _normalize_markov_chain_(self):
    ...
    def fit(self, traj, n_individuals, lid='location'):  # lid = cluster
        # 1. initialize markov chain
        self._create_empty_markov_chain_()

        individuals = traj.uid.unique()
        # fit n individuals trajectory
        with tqdm(total=n_individuals) as pbar:
            for individual in individuals[:n_individuals]:
                # 2. create the time series of the individual
                time_series, individual_shift = self._create_time_series(traj[traj['uid'] == individual],
                                                                         lid=lid)  # start_date, end_date

                # 3. update the markov chain via time_series
                self._update_markov_chain_(time_series, individual_shift)

                pbar.update(1)

        # 4. normalize the markov chain by row
        self._normalize_markov_chain_()

    def _weighted_random_selection(self, weights):
        return np.searchsorted(np.cumsum(weights), random.random())

    def generate(self, diary_length, start_date, random_state=None):
        # temporal diary
        V, i = [], 0
        prev_state = (i, 1)  # initialize (h, R): (0,1)
        ...
        while i < diary_length:
            ...
            if sum(p) == 0:
                next_state = ((prev_state[0] + 1) % 24, prev_state[1])
            else:
        ...
        # mobility diary
        D, other_count = [], 1  # other1, other2, ..., otheri
        D.append([start_date, 0])  # initialize
        ...
        for v in V[1:]:
            ...
            if s == 1:  # home
                ...
                D.append([current_date, 0])
                other_count = 1
            else:

                if h > h_prev:  # same day
                    j = h - h_prev
                else:  # next day
                    j = 24 - h_prev + h
                ...
                for i in range(0, j):
                    ...
                    D.append([current_date, other_count])
                other_count += 1  # other1, other2, ..., otheri

            prev = v

        mobility_diary = []
        prev_location = -1
        for visit_date, abstract_location in D[:diary_length]:
            if abstract_location != prev_location:  # filter continuous same place
                mobility_diary.append([visit_date, abstract_location])
            prev_location = abstract_location

        return pd.DataFrame(mobility_diary, columns=['datetime', 'abstract_location'])
    ...
    **Return phase**. If the individual returns to a previously visited location, such a location :math:`i` is chosen with probability proportional to the number of time the agent visited :math:`i`, i.e., :math:`\Pi_i = f_i`, where :math:`f_i` is the visitation frequency of location :math:`i`.
    ...
    def generate(self, start_date, end_date, spatial_tessellation, gravity_singly={}, n_agents=1,
        ...
        return super().generate(start_date, end_date, spatial_tessellation, gravity_singly=gravity_singly,
                                n_agents=n_agents, starting_locations=starting_locations, od_matrix=od_matrix,
                                relevance_column=relevance_column, random_state=random_state, log_file=log_file,
...
class SpatialEPR(EPR):
    ...
    **Return phase**. If the individual returns to a previously visited location, such a location :math:`i` is chosen with probability proportional to the number of time the agent visited :math:`i`, i.e., :math:`\Pi_i = f_i`, where :math:`f_i` is the visitation frequency of location :math:`i`.
    ...
    def generate(self, start_date, end_date, spatial_tessellation, gravity_singly={}, n_agents=1,
        ...
        return super().generate(start_date, end_date, spatial_tessellation, gravity_singly=gravity_singly,
                                n_agents=n_agents, starting_locations=starting_locations, od_matrix=od_matrix,
                                relevance_column=None, random_state=random_state, log_file=log_file,
                                show_progress=show_progress)
    ...
    **Outline of the DITRAS framework**. DITRAS combines two probabilistic models: a diary generator (e.g., :math:`MD(t)`) and trajectory generator (e.g., d-EPR). The diary generator produces a mobility diary :math:`D`. The mobility diary :math:`D` is the input of the trajectory generator together with a weighted spatial tessellation of the territory :math:`L`. From :math:`D` and :math:`L` the trajectory generator produces a synthetic mobility trajectory :math:`S`.
    ...
    def generate(self, start_date, end_date, spatial_tessellation, gravity_singly={}, n_agents=1,
        ...
        for agent_id in loop:
            ...
            if starting_locations is None:
                self._starting_loc = np.random.choice(np.fromiter(range(num_locs), dtype=int), size=1)[0]
                while sum(od_matrix[self._starting_loc]) == 0.0:
                    self._starting_loc = np.random.choice(np.fromiter(range(num_locs), dtype=int), size=1)[0]
            else:
                self._starting_loc = starting_locations.pop()
