...
from models.utils import get_gps, read_data_from_file
...
class IndividualEval(object):
    ...
    def __init__(self):
        self.X, self.Y = get_gps('../data/shenzhen/location.txt')
        self.max_locs = 2236
        self.max_distance = 0.838
    ...
    def get_geodistances(self, trajs):
        ...
        for traj in trajs:
            for i in range(seq_len - 1):
                lng1 = self.X[traj[i]-1]
                lat1 = self.Y[traj[i]-1]
                lng2 = self.X[traj[i + 1]-1]
                lat2 = self.Y[traj[i + 1]-1]
    ...
    def get_distances(self, trajs):
        ...
        for traj in trajs:
            for i in range(seq_len - 1):
                dx = self.X[traj[i]-1] - self.X[traj[i + 1]-1]
                dy = self.Y[traj[i]-1] - self.Y[traj[i + 1]-1]
    ...
    def get_gradius(self, trajs):
        ...
        seq_len = 48
        for traj in trajs:
            xs = np.array([self.X[t-1] for t in traj])
...
def evaluate(datasets):
    ...
    test_data = read_data_from_file('../data/%s/real.data' % opt.datasets)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datasets',default='shenzhen',type=str)
    opt = parser.parse_args()
    if opt.datasets == 'shenzhen':
        max_locs = 2236
    else:
        max_locs = 23768
