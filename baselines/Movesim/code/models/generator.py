...
class ATGenerator(nn.Module):
    ...
    def __init__(
        ...
        if embedding_net:
            ...
            self.loc_embedding = nn.Embedding(
                num_embeddings=self.total_locations, embedding_dim=self.loc_embedding_dim, padding_idx=0)
    ...
    def forward(self, x_l, x_t):
        ...
        x = self.linear(x)          # (batch_size * seq_len, total_locations)
        x = F.relu(x)   
        ...
        if self.function:
            ...
            pred = x + torch.mul(x,mat1) + torch.mul(x,mat2)    # (batch_size * seq_len, total_locations)
        pred = F.log_softmax(pred, dim=-1)      # (batch_size * seq_len, total_locations)
    ...
    def sample(self, batch_size, seq_len, x=None):
        ...
        if flag:
            ...
            for i in range(given_len, seq_len):
                ...
                x = self.step(x,t)
                x = x.multinomial(1)
        output = torch.cat(samples, dim=1)      # samples=
        return output 
