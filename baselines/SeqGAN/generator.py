...
class Generator(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, use_cuda):
        ...
        self.fc = nn.Linear(hidden_dim, vocab_size)                 # 全连接层(输入大小，输出大小)
    ...
    def forward(self, x):
        ...
        h0, c0 = self.init_hidden(x.size(0))        # x.size(0) = batch_size
    ...
    def step(self, x, h, c):
        """
        Embeds input and applies LSTM one token at a time (seq_len = 1).    x_i
    ...
    def init_hidden(self, batch_size):
        h = torch.zeros(1, batch_size, self.hidden_dim)         # num_layers*n_directions,batch_size,hidden_dim
    ...
    def sample(self, batch_size, seq_len, x=None):
        ...
        if x is None:
            ...
            for _ in range(seq_len):
                ...
                prob = torch.exp(out)
                x = torch.multinomial(prob, 1)          # batch_size * 1
                samples.append(x)
        ...
        # 有输入根据现在的x开始向后采样
            ...
            prob = torch.exp(out)           # 困惑度，1表示完美，无穷大是最差情况。RNN中通常使用困惑度来衡量语言模型的好坏
