'''
定义了我们的oracle model，这个文件不用管，复制过去就好，哈哈。
通过读文件来创建了我们的oracle model，在代码中叫target_lstm
生成一些真实数据，这些数据通过MLE预训练generator
'''
import torch
...
class TargetLSTM(nn.Module):
    ...
    def __init__(self,  vocab_size, embedding_dim, hidden_dim, use_cuda):
        ...
        self.embed = nn.Embedding(vocab_size, embedding_dim)                    # embedding层
    ...
    def forward(self, x):
        ...
        emb = self.embed(x) # out: batch_size * seq_len * emb_dim
        out, _ = self.lstm(emb, (h0, c0)) # out: seq_len * batch_size * (num_directions * hidden_dim)
        out = self.log_softmax(self.fc(out.contiguous().view(-1, self.hidden_dim))) # out: seq_len * batch_size * vocab_size
        return out
